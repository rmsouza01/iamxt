...
char* pytype_string(PyObject* py_obj) {
  if (py_obj == NULL          ) return (char*)"C NULL value";
  if (py_obj == Py_None       ) return (char*)"Python None" ;
  if (PyCallable_Check(py_obj)) return (char*)"callable"    ;
  if (PyString_Check(  py_obj)) return (char*)"string"      ;
  if (PyInt_Check(     py_obj)) return (char*)"int"         ;
  if (PyFloat_Check(   py_obj)) return (char*)"float"       ;
...
char* typecode_string(int typecode) {
  static const char* type_names[25] = {"bool", "byte", "unsigned byte",
				 ...
				 "notype", "char", "unknown"};
  return typecode < 24 ? (char*)type_names[typecode] : (char*)type_names[24];
}
...
%typemap(in,numinputs=0)
  (DATA_TYPE** ARGOUT_ARRAY1, DIM_TYPE* DIM1)
  (DATA_TYPE* data = NULL, int ww, PyObject *array = NULL)
{
    $1 = &data;
    $2 = &ww;
}
...
{
  if(data$argnum == 0) {
    PyErr_SetString(PyExc_RuntimeError, "Cannot allocate array");
    SWIG_fail;  
  }
  npy_intp dims[1] = { (npy_intp) ww$argnum };
  array$argnum = PyArray_SimpleNew(1, dims, DATA_TYPECODE);
  $*1_ltype adata = ($*1_ltype)array_data(array$argnum);
  for(int i = 0; i < ww$argnum; i++) adata[i] = data$argnum[i];
  delete [] data$argnum;
  $result = SWIG_Python_AppendOutput($result, array$argnum);
}
...
%typemap(in,numinputs=0)
  ...
  (DATA_TYPE* data = NULL, int hh, int ww, PyObject *array = NULL)
{
    $1 = &data;
    $2 = &hh;
    $3 = &ww;
  ...
  (DATA_TYPE** ARGOUT_ARRAY2, DIM_TYPE* DIM1, DIM_TYPE* DIM2)
{
  ...
  npy_intp dims[2] = { (npy_intp) hh$argnum,  (npy_intp) ww$argnum };
  array$argnum = PyArray_SimpleNew(2, dims, DATA_TYPECODE);
  $*1_ltype adata = ($*1_ltype)array_data(array$argnum);
  for(int i = 0; i < hh$argnum*ww$argnum; i++) adata[i] = data$argnum[i];
  delete [] data$argnum;
  $result = SWIG_Python_AppendOutput($result, array$argnum);
}

/* ----------------------------------------------------------------------------
 * Typemap suite for (DATA_TYPE** ARGOUT_ARRAY3, DIM_TYPE* DIM1, DIM_TYPE* DIM2, DIM_TYPE* DIM3)
 * ---------------------------------------------------------------------------- */
%typemap(in,numinputs=0)
  (DATA_TYPE** ARGOUT_ARRAY3, DIM_TYPE* DIM1, DIM_TYPE* DIM2, DIM_TYPE* DIM3)
  (DATA_TYPE* data = NULL, int dd, int hh, int ww, PyObject *array = NULL)
{
    $1 = &data;
    $2 = &dd;
    $3 = &hh;
  ...
  (DATA_TYPE** ARGOUT_ARRAY3, DIM_TYPE* DIM1, DIM_TYPE* DIM2, DIM_TYPE* DIM3)
{
  ...
  npy_intp dims[3] = { (npy_intp)dd$argnum, (npy_intp)hh$argnum, (npy_intp)ww$argnum };
  array$argnum = PyArray_SimpleNew(3, dims, DATA_TYPECODE);
  $*1_ltype adata = ($*1_ltype)array_data(array$argnum);
  for(int i = 0; i < dd$argnum*hh$argnum*ww$argnum; i++) adata[i] = data$argnum[i];
  delete [] data$argnum;
  $result = SWIG_Python_AppendOutput($result, array$argnum);
}
